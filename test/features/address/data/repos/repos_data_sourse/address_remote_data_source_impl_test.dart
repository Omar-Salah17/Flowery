...
void main() {
  ...
  setUp(() {
    ...
    addressRemoteDataSource = AddressRemoteDataSourceImpl(apiService: apiServices);
  });
  group('test AddressRemoteDataSourceImpl', () {
     test('when call getLoggedUserAddress it should call getLoggedUserAddress from apiService and when call getLoggedUserAddress from apiService i expect to return LoggedUserAddressModel '
     , () async {
      var addresses = <Addresses>[];
      when(
        apiServices.getLoggedUserAddress(),
      ).thenAnswer((_) async => LoggedUserAddressModel(addresses: addresses));
    var actual = await addressRemoteDataSource.getLoggedUserAddress();
    verify(apiServices.getLoggedUserAddress()).called(1);
    expect(actual, isA<List<Addresses>>());
     });
      test('when call deleteAddress it should call deleteAddress from apiService and when call deleteAddress from apiService i expect to return LoggedUserAddressModel '
     , () async {
      var addresses = <Address>[];
      when(
        apiServices.deleteAddress("addressId"),
      ).thenAnswer((_) async => AddressModel(address:  addresses));    
    var actual = await addressRemoteDataSource.deleteAddress(addressId: "addressId");
